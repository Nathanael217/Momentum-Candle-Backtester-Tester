...
def _scanner_ai_verdict(sig: dict, ml_a: dict = None, ml_b: dict = None,
    ...
    if _QFCOMBOS_OK:
        ...
        if _qf_matches_for_ai is None:
            # Signal not pre-tagged — classify against ALL 5 combos on the fly
        ...
        if _qf_matches_for_ai:
            ...
            _qf_audit_section = (
                ...
                "This signal does NOT match any of the 5 backtest-validated "
                "combos (C6A-N/A, C5B-A/N, C1A-A). The signal may still be "
                "valid based on the per-coin backtest metrics below, but it "
                "lacks the universe-level historical edge confirmation that "
                "the validated combos provide. Apply extra skepticism — the "
                "audit window 2021-11-08 → 2026-04-21 (4.5 yrs · 134 coins · "
                "107,682 trades) found these specific (body × vol × ADX × "
                "regime) profiles to be the most reliable; a non-matching "
                "signal is outside that proven envelope.\n"
...
def render_auto_analyzer(ticker: str, df_full_1d: pd.DataFrame, tc: float,
    ...
    if _QFCOMBOS_OK:
        with st.expander(
            "🎯 QuantFlow Combo Filter — backtest-validated setups (5 combos)",
            expanded=False,
        ):
            st.markdown(
                ...
                f'<b style="color:#58a6ff;">5 backtest-validated combos.</b> '
            ...
            for combo in _qfcombos.COMBOS:
                ...
                pp = combo["primary"]
                checked = st.checkbox(
                    f"**{combo['name']}** (Tier {combo['tier']}) "
                    f"— PF {pf:.2f}, mean R {mr:+.3f} · "
                    f"best plan: {pp['tf'].upper()} {pp['direction']} "
            ...
            if enabled_combos:
                st.caption(
                    f"✅ {len(enabled_combos)} combo(s) active: "
                ...
                st.caption(
                    ...
                    "Tick one or more above to filter to backtest-validated setups."
                )
    ...
    _btc_regime_for_combos = "UNKNOWN"
    ...
    if _QFCOMBOS_OK:
        ...
        for s in all_signals_deduped:
            matches = (_qfcombos.get_matching_combos(
                          s, enabled_combos, btc_regime=_btc_regime_for_combos)
        ...
        if enabled_combos and not all_signals_deduped:
            st.warning(
                ...
                f"({len(enabled_combos)} combo(s): {', '.join(enabled_combos)}). "
                f"Scan + age filter produced {_n_before_combo_filter} signals; "
                f"none satisfied any of the ticked combos' criteria. "
                f"Untick combos to see all signals, or relax body/vol/ADX thresholds."
            )
            return
    ...
    if _QFCOMBOS_OK and enabled_combos:
        _regime_str = (f" · BTC regime: {_btc_regime_for_combos}"
                       if _btc_regime_for_combos != "UNKNOWN"
                       else " · BTC regime: UNKNOWN (-A combos disabled)")
        _combo_filter_suffix = (
            f" &nbsp;|&nbsp; <span style='color:#58a6ff;font-weight:700;'>"
            f"🎯 Combos: {', '.join(enabled_combos)}{_regime_str}"
            f"</span>"
        )
