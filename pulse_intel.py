...
Phase 4 (current):
  ...
  ✓ Smart-money wallet tracker — ERC-20 DEX win-rate scorer (Phase 6)
...
# Phase 6 — Smart Money Wallet Tracker
try:
    import whale_tracker as _wt
    _WHALE_TRACKER_AVAILABLE = True
except ImportError:
    _WHALE_TRACKER_AVAILABLE = False
...
def get_pulse_intel(symbol: str,
    ...
    # Phase 6 — Smart Money Wallet Tracker
    if _WHALE_TRACKER_AVAILABLE:
        whale_signal = _wt.get_token_whale_signal(symbol, etherscan_api_key=etherscan_api_key)
    else:
        whale_signal = {
            "ok": False, "supported": False, "score": 0,
            "label": "MODULE MISSING", "color": "#8892b0",
            "detail": "whale_tracker.py not found — place it in the same directory as pulse_intel.py",
            "data": {},
        }
    ...
    # Phase 6 weights: Flow 25% / TVL 17% / Social 13% / Derivatives 18% / Leaderboard 12% / WhaleSM 15%
    # When Phase 6 (whale_signal) is unavailable, weights revert to Phase 5 distribution.
    # All module weights drop proportionally when a module is N/A (auto-renormalized).
    raw_scores  = []
    raw_weights = []

    if tvl.get("ok") and tvl.get("supported"):
        raw_scores.append(float(tvl["score"]))
        raw_weights.append(0.17)
    ...
    # Phase 6 — Smart Money Wallet Tracker (15% weight)
    _wt_usable = (whale_signal.get("ok") and whale_signal.get("supported")
                  and whale_signal.get("label") not in ("LOW COVERAGE", "NOT MAPPED", "NO DATA", "MODULE MISSING"))
    if _wt_usable:
        raw_scores.append(float(whale_signal["score"]))
    ...
    if _wt_usable:
        _wt_lbl = whale_signal["label"].lower()
        parts.append(f"whale wallets {_wt_lbl}")
    ...
    if _wt_usable:
        _pt_dirs.append(1 if whale_signal["score"] > 0 else (-1 if whale_signal["score"] < 0 else 0))
    ...
    _lb_tag    = " + LBSmartMoney" if _lb_usable else ""
    _wt_tag    = " + WhaleWallets" if _wt_usable else ""
    _phase_n   = "6" if _wt_usable else "5"
    phase_label = (f"{_phase_n} (TVL + {flow_chain} Flow + Social + Macro{_deriv_tag}{_lb_tag}{_wt_tag})"
                   if active_flow else
                   f"{_phase_n} (limited modules{_deriv_tag}{_lb_tag}{_wt_tag})")
    ...
    return {
        ...
        "leaderboard":      leaderboard,        # Phase 5 — Binance Leaderboard proxy
        "whale_signal":     whale_signal,       # Phase 6 — Smart Money Wallet Tracker
        "smart_money":      whale_signal,       # alias: Phase 6 takes over from Phase 5 for this key
